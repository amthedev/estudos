 ...
 *                                             trial_days?, features?[], highlight?, active?, sort_order?, stripe_*? }
 *   PUT    /api/admin/plans/:id
 *   DELETE /api/admin/plans/:id             409 quando há assinaturas vinculadas (desative o plano)
 *   POST   /api/admin/plans/:id/sync-stripe   cria/atualiza produto e preço no Stripe (503 se não configurado)
 *   POST   /api/admin/plans/:id/sync-provider sincroniza com o provedor de pagamento ativo
 *   GET    /api/admin/plans/provider-status   provedor ativo, ambiente e chave mascarada
 *
 * A integração com os provedores vive em services/payments/ (módulo de billing); aqui ela é
...
const planSchema = z
  .object({
    ...
    stripe_product_id: nullable(z.string().trim().max(120)),
    stripe_price_id: nullable(z.string().trim().max(120)),
  })
  .strict();
...
const PLAN_COLUMNS = `p.id, p.slug, p.name, p.description, p.price_cents, p.currency, p.interval, p.interval_count,
  ...
  p.stripe_product_id, p.stripe_price_id, p.provider_plan_id, p.features, p.highlight, p.active, p.sort_order,
...
/** Carrega o serviço do Stripe se existir neste servidor; null caso contrário. */
...
function stripeConfigured(service) {
  if (!service) return false;
  try {
    if (typeof service.status === 'function') {
      const status = service.status();
      if (status && typeof status === 'object' && 'configured' in status) return Boolean(status.configured);
      if (typeof status === 'boolean') return status;
    }
    if (typeof service.isConfigured === 'function') return Boolean(service.isConfigured());
  } catch {
    return false;
  }
  return true;
  ...
  wrap(async (req, res) => {
    ...
    const created = await db.one(
      `INSERT INTO plans (slug, name, description, price_cents, currency, interval, interval_count, trial_days,
                          ...
                          features, highlight, active, sort_order, stripe_product_id, stripe_price_id)
      ...
      [
        ...
        body.stripe_product_id ?? null, body.stripe_price_id ?? null,
      ]
    );
  ...
  wrap(async (req, res) => {
    ...
    await db.query(
      `UPDATE plans SET slug = $1, name = $2, description = $3, price_cents = $4, currency = $5, interval = $6,
              ...
              sort_order = $16, stripe_product_id = $17, stripe_price_id = $18
        WHERE id = $19`,
      [
        slug, body.name, body.description ?? null, body.price_cents, body.currency, body.interval, body.interval_count,
        body.trial_days, body.duration_months, body.bonus_months, body.compare_price_cents ?? null, body.badge || null,
        JSON.stringify(body.features), body.highlight, body.active, body.sort_order,
        body.stripe_product_id ?? null, body.stripe_price_id ?? null, id,
      ]
    );
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    if (!stripe || typeof stripe.syncPlanToStripe !== 'function' || !stripeConfigured(stripe)) {
      throw new AppError(503, 'stripe_unavailable', 'Stripe não configurado. Defina STRIPE_SECRET_KEY no servidor para sincronizar planos.');
    }

    let result;
    try {
      result = await stripe.syncPlanToStripe(plan);
    } catch (err) {
      console.error(`[plans] falha ao sincronizar o plano ${plan.slug} com o Stripe:`, err.message);
      throw new AppError(502, 'stripe_error', `Falha ao sincronizar com o Stripe: ${err.message}`);
    }

    // o serviço pode devolver ids sem gravar — garante a persistência aqui
    if (result && typeof result === 'object') {
      const productId = result.stripe_product_id || result.product_id || (result.product && result.product.id) || null;
      const priceId = result.stripe_price_id || result.price_id || (result.price && result.price.id) || null;
      if (productId || priceId) {
        await db.query(
          `UPDATE plans SET stripe_product_id = coalesce($1, stripe_product_id), stripe_price_id = coalesce($2, stripe_price_id) WHERE id = $3`,
          [productId, priceId, id]
        );
      }
    }

    const updated = await findPlan(id);
    await audit(req, 'plan.sync_stripe', 'plan', id, {
      stripe_product_id: updated.stripe_product_id,
      stripe_price_id: updated.stripe_price_id,
    });
    res.json({ plan: updated, message: 'Plano sincronizado com o Stripe.' });
  })
);
  ...
  wrap(async (req, res) => {
    ...
    if (!(await payments.isConfigured())) {
      throw new AppError(
        ...
        'Nenhum provedor de pagamento configurado no servidor. Defina ASAAS_API_KEY (ou STRIPE_SECRET_KEY) para sincronizar planos.'
      );
    }
